...
public class AdrenoGPUMaxFreqPreference extends CustomSeekBarPreference {

    private static int mDefVal;

    private static final int NODE_LEVEL = R.string.node_gpu_maxfreq_preference;

    public AdrenoGPUMaxFreqPreference(Context context, AttributeSet attrs) {
        super(context, attrs);

        mInterval = context.getResources().getInteger(R.integer.gpu_maxfreq_preference_interval);
        mShowSign = false;
        mUnits = "";
        mContinuousUpdates = false;
        int[] mAllValues = context.getResources().getIntArray(R.array.gpu_maxfreq_preference_array);
    ...
    public static void restore(Context context) {
        ...
        int[] mAllValues = context.getResources().getIntArray(R.array.gpu_maxfreq_preference_array);
        mDefVal = mAllValues[0];
        String storedValue = PreferenceManager.getDefaultSharedPreferences(context).getString(DeviceExtras.KEY_GPU_MAX_FREQ, String.valueOf(mDefVal));
    ...
    private void saveValue(String newValue) {
        ...
        editor.putString(DeviceExtras.KEY_GPU_MAX_FREQ, newValue);
        editor.apply();
    }
